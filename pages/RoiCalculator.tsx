...
import { getRoiReport } from '../services/offlineCalculationService';
import type { ReportData, CalculationRecord, RoiParams } from '../types';
...
const RoiCalculator: React.FC = () => {
  ...
  const handleInputChange = (field: keyof RoiParams, value: string) => {
  ...
  const handleCalculate = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      const data = await getRoiReport(params);
      setReport(data);
      setViewState('report');
  ...
  const resetForm = () => {
    setParams({ initialInvestment: 0, finalValue: 0 });
    setReport(null);
    setError(null);
    setViewState('form');
  };
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div className="bg-white dark:bg-gray-800/50 p-6 md:p-8 rounded-lg border border-gray-200 dark:border-cyan-500/30 shadow-xl">
        <form onSubmit={handleCalculate}>
          <h2 className="text-2xl font-bold text-cyan-700 dark:text-cyan-400 mb-6 text-center">{t('roi.title')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField
              id="initialInvestment"
              label={t('roi.form.initial')}
              type="number"
