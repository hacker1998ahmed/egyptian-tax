...
const StampDutyCalculator: React.FC = () => {
  ...
  const [transactionType, setTransactionType] = useState(STAMP_DUTY_TYPE_KEYS[0]);
  ...
  const transactionTypeOptions = useMemo(() => STAMP_DUTY_TYPE_KEYS.map(key => ({
    value: key,
    label: t(`stampDutyType.${key}` as TranslationKey),
  })), [t]);
  ...
  const handleCalculate = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      const data = await getStampDutyReport(params);
      setReport(data);
      setViewState('report');
  ...
  const resetForm = () => {
    setAmount('');
    setTransactionType(STAMP_DUTY_TYPE_KEYS[0]);
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div className="bg-white dark:bg-gray-800/50 p-6 md:p-8 rounded-lg border border-gray-200 dark:border-cyan-500/30 shadow-xl shadow-gray-400/20 dark:shadow-2xl dark:shadow-cyan-500/10 dark:backdrop-blur-sm">
        <form onSubmit={handleCalculate}>
          <h2 className="text-2xl font-bold text-cyan-700 dark:text-cyan-400 mb-6 text-center">{t('stampDuty.title')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField
              id="amount"
              label={t('stampDuty.form.amount.label')}
              type="number"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              placeholder={t('stampDuty.form.amount.placeholder')}
              required
            />
             <SelectField
              id="year"
            ...
            <div className="md:col-span-2">
              <SelectField
                id="transactionType"
                label={t('stampDuty.form.transactionType.label')}
